    ...
    var body: some View {
        Toggle(isOn: $isDebugLoggingEnabled) {
            HStack(spacing: 4) {
                Text("Debug Logging")
                InfoTip(
                    String(localized: "Writes a detailed record of what Zerm is doing to a file on this Mac. Turn it on only while reproducing a problem you want to report, then turn it off — the log grows quickly and can contain the text of your transcriptions."),
                    doc: .commonIssues
                )
            }
        }
        ...
        if isDebugLoggingEnabled {
            LabeledContent {
                ...
                HStack(spacing: 4) {
                    Text("Debug Log File")
                    InfoTip(String(localized: "Opens the folder holding the debug log so you can read it or attach it to a bug report. Have a look through it before sending it on."))
                }
            }
        }
        ...
        LabeledContent {
            ...
            HStack(spacing: 4) {
                ...
                Text("Export Logs")
                InfoTip(
                    String(localized: "Gathers Zerm's system log entries into one file you can attach to a bug report. This works whether or not Debug Logging is on, but the file is far more useful with it enabled."),
                    doc: .commonIssues
                )
            }
        }
