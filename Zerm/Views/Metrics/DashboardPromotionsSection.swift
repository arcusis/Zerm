...
struct DashboardPromotionsSection: View {
            ...
            HStack(alignment: .top, spacing: 18) {
                if shouldShowUpgradePromotion {
                    DashboardPromotionCard(
                        badge: "30% OFF",
                        title: "Unlock Zerm Pro For Less",
                        message: "Share Zerm on your socials, and instantly unlock a 30% discount on Zerm Pro.",
                ...
                if shouldShowAffiliatePromotion {
                    DashboardPromotionCard(
                        badge: "AFFILIATE 30%",
                        title: "Earn With The Zerm Affiliate Program",
                        message: "Share Zerm with friends or your audience and receive 30% on every referral that upgrades.",
    ...
    private func openSocialShare() {
        if let url = URL(string: "https://tryzerm.com/social-share") {
            NSWorkspace.shared.open(url)
        }
    }
    
    private func openAffiliateProgram() {
        if let url = URL(string: "https://tryzerm.com/affiliate") {
            NSWorkspace.shared.open(url)
        }
    }
